...
const createConnectAccount = async ({ user }) => {
  ...
  const key = process.env.STRIPE_SECRET_KEY;
  const isTestKey = typeof key === "string" && key.startsWith("sk_test_");
  const isLiveKey = typeof key === "string" && key.startsWith("sk_live_");

  const businessUrlRaw = process.env.STRIPE_BUSINESS_URL;
  let businessUrl = null;
  if (isTestKey) {
    // Stripe test mode: do not send business_profile.url.
    // Some Stripe configurations reject business_profile[url] even with example.com.
    businessUrl = null;
  } else {
    if (typeof businessUrlRaw === "string" && businessUrlRaw.trim().length > 0) {
      try {
        const parsed = new URL(businessUrlRaw.trim());
    ...
    if (isLiveKey && !businessUrl) {
      throw new Error(
        "Stripe live mode requires a valid public STRIPE_BUSINESS_URL (must be https and not localhost/private IP)"
      );
    }
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    const mode = isTestKey ? "test" : isLiveKey ? "live" : "unknown";
  ...
  try {
    ...
    const msg =
      typeof rawMessage === "string" && rawMessage.trim().length > 0
        ? typeof rawParam === "string" && rawParam.trim().length > 0
          ? `${rawMessage} (param: ${rawParam})`
          : rawMessage
        : err?.message || "Unable to create Stripe connect account";
    throw new Error(msg);
  }
};
